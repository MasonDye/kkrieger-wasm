        ...
        case REG_TYPE_ADDRESS:
            ...
            else if (shader_is_pixel(ctx))  // actually REG_TYPE_TEXTURE.
            {
                ...
                if (!shader_version_atleast(ctx, 1, 4))
                {
                    ...
                    if (skipGLTexCoord)
                    {
                        // kkrieger-wasm: GLSL ES cannot initialise a global
                        //  from a varying (and ES3 wants constant global
                        //  initialisers), and nothing else declares io_5_N
                        //  for ps_1_x texture registers. Declare the input
                        //  here and copy it at the top of main().
                        ...
                        if (support_glsles(ctx))
                        {
                            ...
                            if (support_glsles3(ctx))
                                output_line(ctx, "in highp vec4 io_%i_%i;",
                                            MOJOSHADER_USAGE_TEXCOORD, regnum);
                            else
                                output_line(ctx, "varying highp vec4 io_%i_%i;",
                                            MOJOSHADER_USAGE_TEXCOORD, regnum);
                            pop_output(ctx);
                        } // if
#endif
                        push_output(ctx, &ctx->mainline_top);
                        output_line(ctx, "%s = io_%i_%i;",
